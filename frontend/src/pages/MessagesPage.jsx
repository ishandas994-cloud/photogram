    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {last && <span style={{ fontSize: 11, color: 'var(--text-3)', flexShrink: 0, marginLeft: 8 }}>{timeAgo(last.created_at)}</span>}
        </div>
...
const MessagesPage = () => {
  ...
  useEffect(() => {
    messagesAPI.getConversations()
      .then(({ data }) => {
        setConvs(data);
        if (convId) setActive(data.find(c => c.id === convId) || null);
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ width: 320, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--surface)', flexShrink: 0 }}>
        <div style={{ padding: '20px 16px 12px' }}>
          <h2 style={{ fontSize: 17, fontWeight: 600, marginBottom: 12 }}>Messages</h2>
          <input
            className="input"
            value={search}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-3)', fontSize: 14 }}>No conversations yet</div>
          ) : (
            filtered.map(conv => (
              <ConvItem
                key={conv.id}
